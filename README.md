...
- **도시 검색**: 도시 이름으로 검색해 위치를 변경할 수 있고, "제주", "서울" 같은 한글 지명도 로마자로 자동 변환해 검색됩니다.
...
- **외부 API**: Open-Meteo(날씨·대기질·지오코딩), BigDataCloud(역지오코딩), Google Gemini API(챗봇)
...
src/
  components/
    chat/        # 날씨 챗봇 위젯
    layout/      # 헤더
    weather/     # 날씨 카드, 시간별 그래프, 도시 검색 등
    ui/          # shadcn 기반 공통 UI 컴포넌트
  context/       # 위치(Location), 온도 단위(Unit) 컨텍스트
  hooks/         # 데이터 조회 및 유틸 훅
  lib/           # API 클라이언트, 포맷터, 날씨 코드/테마 매핑
  types/         # 공용 타입 정의
server/
...
> Gemini API 키는 프론트엔드에 절대 노출되지 않고, `server/index.js` 프록시 서버에서만 사용됩니다. 챗봇 기능 없이 날씨 정보만 쓸 경우 이 단계는 건너뛰어도 됩니다.
